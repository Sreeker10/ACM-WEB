...
function showResults() {
  ...
  // Remove question elements from the DOM
  ...
  scoreDiv.textContent = `Your Score: ${correctAnswers.length} / ${selectedQuestions.length}`;
  resultsDiv.appendChild(scoreDiv);

  // Display correct answers
  const correctHeading = document.createElement("h2");
  correctHeading.textContent = "Correct Answers:";
  resultsDiv.appendChild(correctHeading);

  correctAnswers.forEach((item, index) => {
    const correctDiv = document.createElement("div");
    correctDiv.classList.add("correct-answer");
    correctDiv.textContent = `${index + 1}. ${item.question} - Your Answer: ${
      item.selectedAnswer
    }`;
    resultsDiv.appendChild(correctDiv);
  });

  // Display wrong/unattempted answers
  const wrongHeading = document.createElement("h2");
  wrongHeading.textContent = "Wrong/Unattempted Answers:";
  resultsDiv.appendChild(wrongHeading);

  wrongAnswers.forEach((item, index) => {
    const wrongDiv = document.createElement("div");
    wrongDiv.classList.add("wrong-answer");
    wrongDiv.textContent = `${index + 1}. ${item.question} - Your Answer: ${
      item.selectedAnswer
  ...
  backButton.id = "back-to-main-page-btn";
